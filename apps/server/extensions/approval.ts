...
function parseRoots(): string[] {
  return (process.env.MOWEN_ALLOWED_ROOTS ?? process.env.OHMYPI_ALLOWED_ROOTS ?? "")
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean);
}
...
export default function (pi: ExtensionAPI) {
  pi.on("tool_call", async (event, ctx) => {
    ...
    const mutations = process.env.MOWEN_MUTATIONS ?? process.env.OHMYPI_MUTATIONS ?? "approval";
