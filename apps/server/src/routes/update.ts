...
import { z } from "zod";
import { fetchLatestQingzhouRelease, isQingzhouUpdateAvailable, startQingzhouUpdate } from "../setup/qingzhou-update.js";
...
export function registerUpdateRoutes(
  ...
  app.get("/api/update/latest", async () => {
    ...
    const result = await fetchLatestQingzhouRelease({ env: options.env });
    return {
      current,
      ...
      updateAvailable: isQingzhouUpdateAvailable(result.release?.version, current),
      canUpdate:
        (options.env?.QINGZHOU_DESKTOP === "1" || options.env?.MOWEN_DESKTOP === "1") &&
  ...
  app.post("/api/update/install", async (request, reply) => {
    if (options.env?.QINGZHOU_DESKTOP !== "1" && options.env?.MOWEN_DESKTOP !== "1") {
    ...
    try {
      return await startQingzhouUpdate({ version: parsed.data.version, env: options.env });
    } catch (error) {
