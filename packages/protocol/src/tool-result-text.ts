...
export const TOOL_RESULT_TEXT_MAX = 200_000;
...
export function sanitizeToolResultText(text: string): string {
  ...
  const cleaned = stripped.replace(CONTROL_RE, "");
  if (cleaned.length <= TOOL_RESULT_TEXT_MAX) return cleaned;
  return `${cleaned.slice(0, TOOL_RESULT_TEXT_MAX)}\n…（已截断，共 ${cleaned.length} 字）`;
}
