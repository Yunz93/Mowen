# MyPi one-click installer for Windows.
...
param(
  [string] $Repo = $(if ($env:MYPI_REPO) { $env:MYPI_REPO } else { "Yunz93/ohMyPi" }),
  [string] $Version = $(if ($env:MYPI_VERSION) { $env:MYPI_VERSION } else { "latest" }),
  [switch] $Nightly
)
...
$tmp = Join-Path $env:TEMP "mypi-install"
...
Write-Host "OK  $AppName setup finished. Open MyPi from the Start menu and follow the on-screen guide."
